...
#define IO_U_BUF_OFF(td, i) \
	((((i) + (td)->o.iodepth_batch - 1) / (td)->o.iodepth_batch) * IO_U_BUF_LEN)
...
static int server_open_file(struct thread_data *td, struct fio_file *f)
{
	...
	for (i = 0; i < td->o.iodepth; i++) {
		size_t offset_recv_msg = IO_U_BUF_OFF(td, i) + RECV_OFFSET;
					  ...
					  struct io_u *io_u)
{
	...
	io_u_buf_offset = IO_U_BUF_OFF(td, msg_index);
