...
static void common_td_port(const char *port_base_str,
		struct thread_data *td, char *port_out)
{
	...
	if (port_ul == ULONG_MAX)
		return;
	if (port_ul + td->thread_number - 1 >= UINT_MAX)
		return;

	port_new = port_ul + td->thread_number - 1;
...
static int client_init(struct thread_data *td)
{
	...
	common_td_port(o->port, td, port_td);
