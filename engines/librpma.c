...
static int client_init(struct thread_data *td)
{
	...
	if (td_random(td) || td_rw(td)) {
		sq_size = 2 * td->o.iodepth; /* N x (WRITE + FLUSH) */
	} else if (td_write(td)) {
		if (td->o.sync_io) {
...
static int client_commit(struct thread_data *td)
{
	...
	/* find last read operation */
	if (td_read(td) && cd->io_u_queued_nr != 0) {
		last_read = cd->io_u_queued_nr - 1;
		if (cd->io_us_queued[last_read]->ddir != DDIR_READ) {
			do {
				last_read --;
			} while (!last_read || cd->io_us_queued[last_read]->ddir == DDIR_READ);
		}
	}
	...
	for (i = 0; i < cd->io_u_queued_nr; i++) {
		...
		if (io_u->ddir == DDIR_READ) {
			if (last_read == cd->io_u_queued_nr - 1)
			...
			/* post an RDMA write operation */
			if (td_random(td) || (td_random(td) && td_write(td))) {
				...
				if ((i < cd->io_u_queued_nr - 1) || (cd->io_us_queued[i + 1]->ddir == DDIR_WRITE))
					continue;
