...
static int client_init(struct thread_data *td)
{
	...
	if ((ret = rpma_mr_remote_from_descriptor(&cd->wspace->descriptors[0],
			cd->wspace->mr_desc_size, &cd->server_mr)))
		goto err_conn_delete;
	...
	/* validate the received workspace size */
	cd->wspace_offset = (td->thread_number - 1) * cd->wspace->size;
	if (cd->wspace_offset > server_mr_size ||
			cd->wspace_offset + cd->wspace->size > server_mr_size) {
...
static int client_setup(struct thread_data *td)
{
	...
	f->real_file_size = cd->wspace->size;
...
static inline int client_io_read(struct thread_data *td, struct io_u *io_u, int flags)
{
	...
	size_t src_offset = cd->wspace_offset + io_u->offset;
...
static int server_open_file(struct thread_data *td, struct fio_file *f)
{
	...
	ret = rpma_mr_get_descriptor(mr, &sd->wspace.descriptors[0]);
	if (ret) {
	...
	sd->wspace.mr_desc_size = mr_desc_size;
	sd->wspace.size = td_max_bs(td);

	/* attach the workspace to private data structure */
	sd->pdata.ptr = &sd->wspace;
