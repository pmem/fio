...
static char *server_allocate_pmem(struct thread_data *td, const char *filename, size_t size)
{
	...
	/*
	 * 'mem' can be unaligned. The engine requires aligned io_us
	 * so FIO alignes up the address using the formula below.
	 */
	mem_aligned = PTR_ALIGN(mem_ptr, page_mask) + td->o.mem_align;
	size_aligned = size_mmap - (mem_aligned - mem_ptr);
...
static char *server_allocate_dram(struct thread_data *td, size_t size)
{
	...
	alignment = td->o.mem_align ? td->o.mem_align : page_size;

	if ((ret = posix_memalign((void **)&mem_aligned, alignment, size))) {
		log_err("fio: posix_memalign() failed\n");
		td_verror(td, ret, "posix_memalign");
		return NULL;
	}

	sd->mem_ptr = mem_aligned;
	sd->size_mmap = 0;
...
static int server_open_file(struct thread_data *td, struct fio_file *f)
{
	...
	if (strcmp(f->file_name, "malloc") == 0) {
		...
		io_us_ptr = server_allocate_dram(td, io_us_size);
	} else {
		/* allocation from PMEM using pmem_map_file() */
		io_us_ptr = server_allocate_pmem(td, f->file_name, io_us_size);
		f->real_file_size = sd->size_mmap;
	}

	if (io_us_ptr == NULL)
		return 1;

	ret = rpma_mr_reg(sd->peer, io_us_ptr, io_us_size,
			RPMA_MR_USAGE_READ_DST | RPMA_MR_USAGE_READ_SRC |
			RPMA_MR_USAGE_WRITE_DST | RPMA_MR_USAGE_WRITE_SRC |
			RPMA_MR_USAGE_FLUSH_TYPE_PERSISTENT,
			&mr);
