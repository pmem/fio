		...
		unsigned long long int len)
{
	...
	src_offset = ccd->io_us_size; /* source offset of the AOF pointer */
	pointer_ptr = (uint64_t *)ccd->orig_buffer_aligned + src_offset; /* address of the AOF pointer */
	*pointer_ptr = first_io_u->offset + len; /* value of the AOF pointer */

	if ((ret = rpma_write_atomic(ccd->conn, ccd->server_mr, dst_offset,
			ccd->orig_mr, src_offset, RPMA_F_COMPLETION_ON_ERROR,
			(void *)(uintptr_t)last_io_u->index))) {
	...
	if ((ret = rpma_flush(ccd->conn, ccd->server_mr, dst_offset, sizeof(uint64_t),
