...
struct fio_option librpma_aof_options[] = {
	LIBRPMA_FIO_OPTIONS_COMMON,
	{
		...
		.posval = {
			{
				.ival = "classic",
				.oval = LIBRPMA_AOF_MODE_CLASSIC,
				.help = "classic",
			},
			{
				.ival = "XXX",
				.oval = LIBRPMA_AOF_MODE_XXX,
				.help = "XXX",
			},
		},
...
static int server_classic_init(struct thread_data *td);
static int server_classic_post_init(struct thread_data *td);
static int server_classic_open_file(struct thread_data *td, struct fio_file *f);
static enum fio_q_status server_classic_queue(struct thread_data *td,
		struct io_u *io_u);
static void server_classic_cleanup(struct thread_data *td);

static int server_xxx_open_file(struct thread_data *td, struct fio_file *f);
static enum fio_q_status server_xxx_queue(struct thread_data *td,
		struct io_u *io_u);
...
static int server_init(struct thread_data *td)
{
	...
	if (o->aof_mode == LIBRPMA_AOF_MODE_CLASSIC) {
		if ((ret = server_classic_init(td))) {
			librpma_fio_server_cleanup(td);
			return ret;
		}

		td->io_ops->post_init = server_classic_post_init;
		td->io_ops->open_file = server_classic_open_file;
		td->io_ops->queue = server_classic_queue;
		td->io_ops->cleanup = server_classic_cleanup;
	} else { /* LIBRPMA_AOF_MODE_XXX */
		td->io_ops->open_file = server_xxx_open_file;
		td->io_ops->queue = server_xxx_queue;
...
static int server_classic_prepare_connection(struct thread_data *td,
...
static int server_classic_open_file(struct thread_data *td, struct fio_file *f)
{
	...
	csd->prepare_connection = server_classic_prepare_connection;
...
static int server_classic_qe_process(struct thread_data *td,
...
static inline int server_classic_queue_process(struct thread_data *td)
{
	struct librpma_fio_server_data *csd = td->io_ops_data;
	struct server_data *sd = csd->server_data;
	int ret;
	...
	for (i = 0; i < qes_to_process; ++i) {
		if ((ret = server_classic_qe_process(td, &sd->msgs_queued[i])))
			return ret;
	}
...
static int server_classic_cmpl_process(struct thread_data *td)
{
	struct librpma_fio_server_data *csd = td->io_ops_data;
	struct server_data *sd = csd->server_data;
...
{
	do {
		if (server_classic_cmpl_process(td))
			return FIO_Q_BUSY;

		if (server_classic_queue_process(td))
			return FIO_Q_BUSY;
...
FIO_STATIC struct ioengine_ops ioengine_server = {
	 ...
	 *    according to the picked aof_mode value
	 */
};
